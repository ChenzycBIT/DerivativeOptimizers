...
comparing_data = []
learning_rates = [0., 0.001, 0.002, 0.003, 0.005, 0.01]
...
if derivative_sign == True:
    for i in range(len(derivatives)):
        for j in range(20):
            if j == 0:
                acc_data = np.array(training(model_sign=model_sign, optimizer_sign=optimizer_sign, learning_rate=learning_rates[0],
