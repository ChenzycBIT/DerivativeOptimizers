        ...
        for i, (images, labels) in enumerate(train_loader):
            if i % 100 == 0 and basicgrad_sign == True:
                for j, (all_images, all_labels) in enumerate(BGD_loader):
                    ...
                    outputs = nn.parallel.data_parallel(net, all_images, device_ids=[0, 1, 2, 3])
            ...
            optimizer.zero_grad()  # zero the gradient buffer
            outputs = nn.parallel.data_parallel(net, images, device_ids=[0, 1, 2, 3])
...
testing_algorithms = [0, 1, 2, 3, 4, 5]
